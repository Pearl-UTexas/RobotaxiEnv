...
class Field(object):
    ...
    def __setitem__(self, point, cell_type):
        ...
        if cell_type == CellType.EMPTY or cell_type == CellType.DUMMY:
    ...
    def create_level(self):
        ...
        try:
            ...
            self._empty_cells = {
                ...
                for y in range(self.size)
                for x in range(self.size)
                if self[(x, y)] == CellType.EMPTY or self[(x, y)] == CellType.DUMMY
            }
            self._dummy_cells = {
                Point(x, y)
                for y in range(self.size)
                for x in range(self.size)
                if self[(x, y)] == CellType.DUMMY
            }
    ...
    def get_initial_items(self):
        return list(self._good_fruits), list(self._bad_fruits), list(self._lavas)
    ...
    def update_snake_footprint(self, old_head, old_tail, new_head):
        ...
        if old_tail:
            if old_tail in self._dummy_cells:
                self[old_tail] = CellType.DUMMY
            else:
